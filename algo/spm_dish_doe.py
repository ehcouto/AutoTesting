...
class SpmDishConfig:
    def __init__(self,
                 ...
                 off_time=None,
                 enable_heating=None,
                 heating_timeout=None,
                 IsAlternatingWash=None,
                 log_to_file=None,
                 log_level=None):
        ...
        self.heating_timeout = heating_timeout
        self.IsAlternatingWash = IsAlternatingWash
    ...
    def parser(self, config):
        ...
        try:
            self.voltages = [int(i.strip()) for i in config.get('SpmDishDoeConfig', 'Voltages').split(',')]
            self.frequencies = [int(i.strip()) for i in config.get('SpmDishDoeConfig', 'Frequencies').split(',')]
            self.on_time = config.getint('SpmDishDoeConfig', 'OnTimeSec')
            self.off_time = config.getint('SpmDishDoeConfig', 'OffTimeSec')
            self.n_startups = config.getint('SpmDishDoeConfig', 'NumberStartups')
            self.enable_heating = config['SpmDishDoeConfig'].getboolean('enable_heating')
            self.heating_timeout = config.getint('SpmDishDoeConfig', 'heating_timeout')
            self.IsAlternatingWash = config['SpmDishDoeConfig'].getboolean('IsAlternatingWash')
...
class SpmDishTest(object):
    ...
    def __init__(self, config_path):
        ...
        self.test_config = SpmDishConfig()
    ...
    def run_test(self, tb, test_param_name=None, test_param_value=None, conv_formula=None):
        ...
        if self.test_config.enable_heating:  
            est_time = (((self.test_config.on_time+1) + (self.test_config.off_time+1))*(self.test_config.n_startups)*len(self.test_config.frequencies)*len(self.test_config.voltages))+self.test_config.heating_timeout
        else:
            est_time = ((self.test_config.on_time+1) + (self.test_config.off_time+1))*(self.test_config.n_startups)*len(self.test_config.frequencies)*len(self.test_config.voltages)
        
        tb.logger.info('Expected test duration: ' + str(datetime.timedelta(seconds=est_time)))
         
        tb.start_acquisition()
        
        fail_counter_recovery = 0
        fail_counter_fault = 0
        fail_types = []
        time_to_start = 0
        time_to_start_min = 0
        time_to_start_max = 0
        ...
        for line_voltage in self.test_config.voltages:
            ...
            for line_frequency in self.test_config.frequencies:
                ...
                fail_counter_recovery = 0.0
                fail_counter_fault = 0.0
                fail_types.clear()
                ...
                if self.test_config.enable_heating:                              
                    self.PreTest_Heating_Pump(tb)
                
                
                tb.logger.info('**** GWS STARTS ****')
                for s_run in range(self.test_config.n_startups):
                ...
                report_table['Total Startups'].append(self.test_config.n_startups)
                report_table['# of Recoveries'].append(fail_counter_recovery)
                report_table['# of Faults'].append(fail_counter_fault)
                report_table['Success Percentage'].append((self.test_config.n_startups - fail_counter_recovery)/self.test_config.n_startups*100)
                ...
                report_table['Time to Start (AVR) [Sec]'].append(time_to_start_tmp/self.test_config.n_startups)
                
                tb.logger.info('AVERAGE TIME TO START [Sec]: ' + str(time_to_start_tmp/self.test_config.n_startups) + '*****')
    ...
    def PreTest_Heating_Pump(self, tb):
        tb.logger.info('**** PRE START - HEATING THE MOTOR! ****')
        tb.write_board_var('My_Start_Motor', 1)
        time.sleep(self.test_config.heating_timeout) #Keep motor running for while (heating_timeout seconds)
