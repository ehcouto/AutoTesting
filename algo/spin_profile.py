...
class SpinProfileTest():
    ...
    def run_test(self, tb, test_param_name=None, test_param_value=None, conv_formula=None):
        ...
        info_time_init = np.int(5.0/self.test_config.control_step_delay)
        info_time = info_time_init
        ...
        while impulse_index < len(self.test_config.target_power):
            ...
            if info_time == 0:
                ...
                tb.logger.info('Bridge Temperature: %f', tb.read_instr_track('Bridge'))
                tb.logger.info('PFC Temperature: %f', tb.read_instr_track('PFC'))
                tb.logger.info('IGBT2 Temperature: %f', tb.read_instr_track('IGBT2'))
                tb.logger.info('Ambient Temperature: %f', tb.read_instr_track('Temp_Ext'))
                tb.logger.info('Time spin: %f', time_spin)
