...
#             at_obj.logger.info('Iq limit is: %f', iq_limit)
        ...
        speed = at_obj.read_instr_track('speed')
        elapsed_time = 0
        while ((speed > 5) and (elapsed_time < 80)):
            speed = at_obj.read_instr_track('speed')
            at_obj.logger.debug('Stopping speed: %f', speed)
            time.sleep(2)
            elapsed_time += 2
        if elapsed_time > 80:
...
def speed_torque_rising_acquire(at_obj, test_config, speed, speed_thr_perc, torque_min, torque_step, param_name=None, param_value=None, conv_formula=None):
        ...
        if speed <= 15500.0:
            ...
            actual_speed = at_obj.read_board_var('Mcl_Quantities.Speed_Rot_Mech') #Use speed from the board... 
        ...
        while actual_speed > SPEED_THR_LO:
            ...
            speed_rads = (actual_speed * np.pi) / 30.0
            shaft_power = speed_rads * torque
            at_obj.logger.info('Motor Shaft Power [W]: %f', shaft_power)
            at_obj.logger.info('Read speed is: %s', actual_speed)
            torque += torque_step
...
def measure_from_speed_torque_dtc_acquisition(at_obj, test_config, acquisition, n_sub_acquisition, steady_speed,config):
    ...
    for marker_value in range(1, n_sub_acquisition):
        ...
        if skip_measure == False:
            ...
            if steady_speed < 15500:
                tr_Speed = acq_to_process.get_track('speed')
            else:
                tr_Speed = tr_f16Omega
...
def motor_heating(at_object, config):
    ...
    while sec <= timeout:
        ...
        time.sleep(SAMPLING_TIME)
        sec = time.time() - sec_init 
        at_object.logger.info('Elapsed time: %f', sec)
        at_object.set_motor_speed(target_speed, acceleration)
