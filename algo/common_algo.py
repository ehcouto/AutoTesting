...
def speed_torque_acquire(at_obj, test_config, speed, torque_ramp_values, tr=None, param_name=None, param_value=None, conv_formula=None):
        ...
        #at_obj.write_board_var('My_Measured_Temp', 40.0) #Update Temperature in the F/W.
...
#             time.sleep(test_config.steady_state_time)
        ...
        CLASSB_MAX_TEMP = 180.0
        TEMP_LIMIT = 145.0  #Thermocouples
        Min_CoolDown_Temp = 100.0
        Curr_ki = 0.5
        Curr_k0 = 0.001
        current_setting = 0.92 #0.80  #40% of rated current @ rated current = 2Arms
        current_tolerance_thr = 0.003 #0.3%
        current_tolerance = current_setting*current_tolerance_thr
        torque = 0.7
        
        tracking_temp_time = 5*60 #5mins
        tracking_temp_step = 2
        tracking_temp_limit = 20 #Init at 30C
        tracking_temp_count = 0
       
        total_time_sec = 6.0 * 60.0 * 60.0 #4h
...
#             #sanity check on the temperature readings
#             if sample_of_motor_temp > (TEMP_LIMIT+1.0) or sample_of_motor_temp < 0.0:
#                 sample_of_motor_temp = (sample_of_motor_temp_2+sample_of_motor_temp_3)/2.0
#             if sample_of_motor_temp_2 > (TEMP_LIMIT+1.0) or sample_of_motor_temp_2 < 0.0:
#                 sample_of_motor_temp_2 = (sample_of_motor_temp+sample_of_motor_temp_3)/2.0
#             if sample_of_motor_temp_3 > (TEMP_LIMIT+1.0) or sample_of_motor_temp_3 < 0.0:
#                 sample_of_motor_temp_3 = (sample_of_motor_temp_2+sample_of_motor_temp)/2.0
            ...
            #----- Current Controller -----#
            at_obj.logger.info('Current reference: %f', current_setting)
            curr_err = current_setting - sample_of_motor_current
            curr_err_abs = abs(curr_err)
            curr_err_sign = math.copysign(1, curr_err)
            at_obj.logger.debug('Current error: ' + str(curr_err))
            torque_step = (Curr_ki * curr_err_abs + Curr_k0) * curr_err_sign;
            at_obj.logger.debug('Torque step: ' + str(torque_step))
            
            if curr_err_abs > current_tolerance:
                torque = torque + torque_step     #update torque
                at_obj.dsp6000_setup(torque)
...
#                 max_temp = sample_of_motor_temp_3
            ...
            #at_obj.write_board_var('My_Measured_Temp', max_temp+10.0) #Update Temperature in the F/W.
            
            if max_temp > tracking_temp_limit:
                tracking_temp_limit = max_temp + tracking_temp_step #increment the tracking temperature...
                tracking_temp_count = 0
			
            if tracking_temp_count >= tracking_temp_time: # temperature stabilized...
                current_setting = current_setting*1.1
                current_tolerance = current_setting*current_tolerance_thr
                tracking_temp_count = 0
                tracking_temp_limit = max_temp + tracking_temp_step #increment the tracking temperature...
            ...
            at_obj.logger.info('Temperature Limit [C]: %f', tracking_temp_limit)
            at_obj.logger.info('Temperature Limit Counter [Secs]: %f', tracking_temp_count)
            at_obj.logger.info('Estimated ClassA Temperature [C]: %f', at_obj.read_board_var('Mcl_Quantities.Stator_Temperature'))
            ...
            if mci_sr_fault != 0:
                at_obj.dsp6000_setup(0)
                at_obj.logger.info('FAULT TRIGGERED!!!... STOPPING THE TEST!!!')
                break
            
            if classb_temp > CLASSB_MAX_TEMP:
                at_obj.logger.info('PROTECTION REACHED... STOPPING THE MOTOR!!!')
                break
        ...
        if mci_sr_fault == 0:
            at_obj.set_motor_speed(0, test_config.acceleration) #If not failed...
        # make sure that we actually stop the motor
        
        elapsed_time = 0
        STEP_TIME = 5
        while ((max_temp > Min_CoolDown_Temp) and (elapsed_time < 1800)):
            time.sleep(STEP_TIME)
            elapsed_time += STEP_TIME
            sample_of_motor_temp = at_obj.read_instr_track('motor_temperature')
            sample_of_motor_temp_2 = at_obj.read_instr_track('motor_temperature_2')
            sample_of_motor_temp_3 = at_obj.read_instr_track('motor_temperature_3')
            at_obj.logger.info('******* COOLING DOWN *******')
            at_obj.logger.info('Elapsed Time [Secs]: %f', elapsed_time)
            at_obj.logger.info('Measured Temperature [C]: %f', sample_of_motor_temp)
            at_obj.logger.info('Measured Temperature_2 [C]: %f', sample_of_motor_temp_2)
            at_obj.logger.info('Measured Temperature_3 [C]: %f', sample_of_motor_temp_3)
...
#                 max_temp = sample_of_motor_temp_3
            ...
            #at_obj.write_board_var('My_Measured_Temp', max_temp) #Update Temperature in the F/W.

        if elapsed_time >= 7200:
            raise RuntimeError('STOPPING.... TOO LONG COOL DOWN...')
