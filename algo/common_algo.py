...
def speed_torque_acquire(at_obj, test_config, speed, torque_ramp_values, tr=None, param_name=None, param_value=None, conv_formula=None):
        ...
        #at_obj.write_board_var('My_Measured_Temp',35.0) #Update Temperature in the F/W.
...
#             time.sleep(test_config.steady_state_time)
        ...
        Max_Time_Cool_Down = 3600*3 #3h
        if speed < 2000: 
            torque = 0.5
            Curr_ki = 0.5
            Curr_k0 = 0.001
            current_setting = 1.50 #0.80  #40% of rated current @ rated current = 2Arms
            current_tolerance_thr = 0.003 #0.3%
            current_tolerance = current_setting*current_tolerance_thr           
        else:
            torque = 0.18
        
        total_time_sec = 180.0 * 60.0 #Limit: 3h
        
        tracking_temp_time = (5*60)+6 #5mins + 1%
        tracking_temp_step = 1
        tracking_temp_limit = 20
        ...
        while elapsed_time < total_time_sec:
            ...
            if speed > 2000:
                sample_of_motor_speed = at_obj.read_board_var('Mcl_Quantities.Speed_Rot_Mech')
                sample_of_motor_speed_rads = sample_of_motor_speed * 2 * np.pi / 60.0
                sample_of_toque = abs(at_obj.read_instr_track('torque'))
                
                shaft_power = sample_of_toque * sample_of_motor_speed_rads
                at_obj.logger.info('*** Shaft Power [W]: %f ***', shaft_power)
            ...
            if tracking_temp_count >= tracking_temp_time: # temperature stabilized...
                tracking_temp_count = 0
                tracking_temp_time = 100000
                tracking_temp_limit = 200 #increment the tracking temperature...
                total_time_sec = elapsed_time + 361 #+6minutes
                #break
            ...
            if speed > 2000:
                if sample_of_motor_speed == 0: #If motor stops, safe escape...
                    at_obj.logger.info('ABORTING... MOTOR HAS SUDDENLY STOPPED!')
                    break
        ...
        at_obj.logger.info('Measured Temperature_2 [C]: %f', sample_of_motor_temp_2)
        at_obj.logger.info('Measured Temperature_3 [C]: %f', sample_of_motor_temp_3)
        #at_obj.logger.info('Measured Temperature [C]: %f', max_temp)
        at_obj.logger.info('Temperature Limit [C]: %f', tracking_temp_limit)
        at_obj.logger.info('Temperature Limit Counter [Secs]: %f', tracking_temp_count)
        at_obj.logger.info('Estimated ClassA Temperature [C]: %f', at_obj.read_board_var('Mcl_Quantities.Stator_Temperature'))
        at_obj.logger.info('Estimated ClassB Temperature [C]: %f', at_obj.read_board_var('SR_Overheating_Temp'))
        ...
        while (max_temp > 30.0):
            ...
            if elapsed_time >= Max_Time_Cool_Down: 
                at_obj.logger.info('STOPPING.... TOO LONG COOL DOWN...')
                break
