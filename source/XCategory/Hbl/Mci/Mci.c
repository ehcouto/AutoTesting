...
//    API220_RUN_CALLBACK(motor, speed_rpm, ramp_rpm_per_s, response);
    ...
    if(speed_rpm)
    {
    	current_pk = (sint32)((float32)speed_rpm*0.1f);
    	freq = (sint32)(ramp_rpm_per_s);
    	response = Mci__Manual_Injection(motor, MCI_MANUAL_INJECTION_AC_CURRENT, current_pk, current_pk, freq, freq);
    }
    else if(speed_rpm == 0)
    {
    	response = Mci__Stop(MOTOR0, 10000);
    }
    return(response);
}
