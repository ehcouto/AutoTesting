...
def win_log_extract(drive,file_id):
  ...
  win_log_data = StringIO(str(acq_data,encoding = 'utf_8'))
  win_log_data_new = win_format_change(win_log_data)
  
  win_log_data_df = pandas.read_csv(StringIO(str(win_log_data_new,encoding = 'utf_8')))
...
def win_format_change(win_file_name): 
    ...
    for win_packet in win_log_reader:
        if not(win_packet == '\n'):
            ...
            win_packet_string_new = win_packet_string_new[0:2] + "/" +  win_packet_string_new[4:6] +  "/" +  win_packet_string_new[8:] ##ONLY FOR POLAND
...
def process_df(df,process_type):
    ...
    elif process_type == 'MCU_Anomalous_Reset':
        results = process_MCU_anomalous_resets(df)
...
#getting ILLEGAL OPCODE Resets from MCU (API09)    
def process_MCU_anomalous_resets(df):
    pandas.options.mode.chained_assignment = None  # default='warn'
    results = df[(df['API'] == 9) & (df['Source'] == 0) & (df['Opcode'] == 1) & (df['Payload'].str.startswith('10',na=False))]
    return results    

#return the recap string
def recap_results(results,process_type, result_type = 'raw'):
...
def decode_MCI_error(code):  # it expect an int value
    error = []
    for key,value in MCI_ERRORS_DICT.items():
